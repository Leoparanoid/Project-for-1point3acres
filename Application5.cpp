...
int Application5::Initialize()
{
	...
	status |= GzNewDisplay(&m_fDisplay, m_nWidth, m_nHeight);

	status |= GzGetDisplayParams(m_fDisplay, &xRes, &yRes); 

	status |= GzInitDisplay(m_fDisplay);
 
	//status |= GzNewRender(&m_pRender, m_fDisplay); 
...
int Application5::Render() 
{
	...
	*/ 
	...
	m_pDisplay_Merge();
	GzFlushDisplay2File(outfile, m_fDisplay); 	/* write out or update display to file*/
	GzFlushDisplay2FrameBuffer(m_pFrameBuffer, m_fDisplay);	// write out or update display to frame buffer
...
int Application5::Clean()
{
	...
	status |= GzFreeDisplay(m_fDisplay);
...
int Application5::m_pDisplay_Merge() {

	for (int i = 0; i < AAKERNEL_SIZE; i++)
		if (!m_pDisplay[i])
			return GZ_FAILURE;

	if (!m_fDisplay)	return GZ_FAILURE;

	for (int i = 0; i < m_fDisplay->yres; i++)
		for (int j = 0; j < m_fDisplay->xres; j++) {
			int index = (j + (i * m_fDisplay->xres));
			m_fDisplay->fbuf[index].red = 0;
			m_fDisplay->fbuf[index].green = 0;
			m_fDisplay->fbuf[index].blue = 0;
			m_fDisplay->fbuf[index].alpha = 0;
			m_fDisplay->fbuf[index].z = 0;
		}
	...
	for (int i = 0; i < m_fDisplay->yres; i++)
		for (int j = 0; j < m_fDisplay->xres; j++)
			for (int k = 0; k < AAKERNEL_SIZE; k++) {
				int index = (j + (i * m_fDisplay->xres));
				m_fDisplay->fbuf[index].red += m_pDisplay[k]->fbuf[index].red * AAFilter[k][2];
				m_fDisplay->fbuf[index].green += m_pDisplay[k]->fbuf[index].green * AAFilter[k][2];
				m_fDisplay->fbuf[index].blue +=  m_pDisplay[k]->fbuf[index].blue * AAFilter[k][2];
				m_fDisplay->fbuf[index].alpha +=  m_pDisplay[k]->fbuf[index].alpha * AAFilter[k][2];
				m_fDisplay->fbuf[index].z += m_pDisplay[k]->fbuf[index].z * AAFilter[k][2];
			}
	return GZ_SUCCESS;
}